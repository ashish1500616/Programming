...
void ss(vector<int> &A, vector<vector<int>> &ans, vector<int> &subs, int index, int k)
{
    if (index >= A.size() || accumulate(subs.begin(), subs.end(), 1ll * 0) == k)
    {
        if (accumulate(subs.begin(), subs.end(), 1ll * 0) == k)
...
void findUniquePermutation(vector<int> &A, vector<vector<int>> &a, int index, int ns)
{
    map<int, int> mp;
    if (index == ns)
    {
    ...
    for (int i = index; i < ns; i++)
    {
        ...
        mp.insert({A[i], 1});
        swap(A[i], A[index]);
        findUniquePermutation(A, a, index + 1, ns);
        swap(A[i], A[index]);
    }
    return;
}
vector<vector<int>> Solution::permute(vector<int> &A)
{
    vector<vector<int>> ans;
    int s = A.size();
    findUniquePermutation(A, ans, 0, s);
    return ans;
}
